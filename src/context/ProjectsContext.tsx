...
interface ProjectsContextValue {
  ...
  deleteProject: (projectId: string, membersToFork?: MemberWithProfile[]) => Promise<{ error: Error | null }>; // Soft delete (moves to trash, forks for members if provided)
...
`;
  ...
  const forkProjectForUser = useCallback(async (
    projectId: string,
    targetUserId: string,
    ...
    try {
      ...
      const forkSuffix = originalOwnerName ? ` - from ${originalOwnerName}` : " (fork)";
      ...
      if (files && files.length > 0) {
        const newFiles = files.map((file: { id: string; name: string; language: string; content: string }) => {
  ...
  const deleteProject = useCallback(async (projectId: string, membersToFork?: MemberWithProfile[]) => {
    if (!supabase || !user) {
    ...
    try {
      ...
      if (membersToFork && membersToFork.length > 0) {
        ...
        const ownerName = profile?.display_name || user.user_metadata?.name || "Previous Owner";
        ...
        console.log(`Created ${forkResults.length - failedForks.length} forks successfully`);
      }
  ...
  }, [supabase, user, profile, currentProjectId, projects, forkProjectForUser]);
  ...
  // Requires RLS policy: "Admins can update any project"
