...
export class AnnotationWidget extends WidgetType {
  ...
  updateDOM(dom: HTMLElement, view: any): boolean {
    // Handle reply input state changes without recreating the entire widget
    if (!this.expandedAnnotationId || this.expandedAnnotationId !== this.annotation.id) {
      return false; // Not expanded, no reply section to update
    }

    const repliesSection = dom.querySelector('.cm-annotation-replies');
    if (!repliesSection) {
      return false; // No replies section, need full recreation
    }

    // Get the reply form container or add-reply button
    const existingForm = repliesSection.querySelector('.cm-annotation-reply-input');
    const existingAddBtn = repliesSection.querySelector('.cm-annotation-add-reply-btn');
    const isInputOpen = this.replyInputOpenFor === this.annotation.id;

    // If input should be open but isn't
    if (isInputOpen && !existingForm) {
    ...
    // If input should be closed but is open
    if (!isInputOpen && existingForm) {
      const formParent = existingForm.closest('div');
