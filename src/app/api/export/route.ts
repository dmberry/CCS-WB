...
export async function POST(request: NextRequest) {
  try {
    ...
    if (format === "pdf") {
      ...
      return new NextResponse(new Uint8Array(pdfBuffer), {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="ccs-lab-session-${new Date().toISOString().slice(0, 10)}.pdf"`,
        },
      });
    }
    ...
    return NextResponse.json(exportData, {
      status: 200,
      headers: {
        "Content-Disposition": `attachment; filename="ccs-lab-session-${new Date().toISOString().slice(0, 10)}.json"`,
      },
    });
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    message: "Export endpoint for CCS-lab sessions",
    usage: {
      method: "POST",
      body: {
