...
export default function ConversationPage() {
  ...
  const handleExportConversation = () => {
    ...
    a.href = url;
    a.download = `ccs-lab-session-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleExportOutputsOnly = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `ccs-lab-critiques-${new Date().toISOString().slice(0, 10)}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleExportOutputsPDF = () => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setTextColor(150, 150, 150);
      doc.text(
        `Page ${i} of ${pageCount} | Generated by CCS-lab`,
        pageWidth / 2,
        pageHeight - 10,
        { align: "center" }
      );
    ...
    // Save the PDF
    doc.save(`ccs-lab-critiques-${new Date().toISOString().slice(0, 10)}.pdf`);
    setShowExportModal(false);
  ...
  const handleExportPDF = () => {
    ...
    // Save the PDF
    doc.save(`ccs-lab-session-${new Date().toISOString().slice(0, 10)}.pdf`);
    setShowExportModal(false);
